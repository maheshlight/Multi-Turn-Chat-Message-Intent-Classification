...
def get_anthropic_client():
    ...
    return Anthropic()   # uses env var automatically
...
# ─────────────────────────────────────────────────────────────────────────────
# Claude API prediction — single text
# ─────────────────────────────────────────────────────────────────────────────
def analyze_with_claude_single(text: str) -> dict:
    system_prompt = (
        "You are an expert NLP intent classification system. "
...
# ─────────────────────────────────────────────────────────────────────────────
# Claude API prediction — multi-turn
# ─────────────────────────────────────────────────────────────────────────────
def analyze_with_claude_multiturn(turns: list) -> dict:
    system_prompt = (
        "You are an expert NLP intent classification system specializing in multi-turn dialogue. "
...
with st.sidebar:
    ...
    engine = st.radio(
        ...
        ["Claude AI (Anthropic)" if True else "Claude AI",
         "ML Model (TF-IDF)" if ML_AVAILABLE else "ML Model (not loaded)"],
        index=0,
    )
    use_claude = "Claude" in engine
    ...
    if st.session_state.turns:
        ...
        if st.button("🔍 Analyze Conversation", type="primary"):
            with st.spinner("Analyzing conversation…"):
                ...
                # ── Claude API
                with claude_col:
                    st.markdown("#### 🤖 Claude AI — Deep Analysis")
                    try:
...
# ─────────────────────────────────────────────────────────────────────────────
st.divider()
st.caption("🧠 Multi-Turn Chat Intent Classifier · Team 1 · Module 7 NLP · Powered by Anthropic Claude + Scikit-learn")
